...
// Callback para filtrar los datos de la tabla y detalle
    ...
    $("#selectall").prop("checked", false);
        ...
        if(quincena != '' || usuario != ''  || contrato != '' || ips != ''){

            $('#tturnos').DataTable().destroy();

            fill_datatable_tabla(quincena, usuario, ips, contrato);
...
// Función para filtrar cargar los datos en la tabla
 ...
 function fill_datatable_tabla(quincena = '', usuario = '', ips = '', contrato = '' )
         {
          ...
          ajax:{
               url:"{{route('liquidinfo')}}",
                data:{quincena:quincena,ips:ips, usuario:usuario, contrato:contrato }
              },
              columns: [
          {data:'action',
           orderable: false},
               ...
               "columnDefs": [
                 ...
                 "createdRow": function(row, data, dataIndex) {
                    if (data["type_salary"] == 'FIJO-QUINCENAL-MENSUAL') {
                        $($(row).find("td")[6]).addClass("btn btn-sm bg-info rounded-lg");
                    }else{
                        $($(row).find("td")[6]).addClass("btn btn-sm bg-dark rounded-lg");
                        }
                    if (data["total_pagar"] > 1) {
                    $($(row).find("td")[15]).addClass("btn btn-sm bg-success rounded-lg");
                  }
                  if (data["parafiscales"] > 1) {
                        $($(row).find("td")[10]).addClass("btn btn-sm bg-danger rounded-lg ");
                    }
                    if (data["retencion"] >= 0) {
                        $($(row).find("td")[12]).addClass("btn btn-sm bg-warning rounded-lg ");
                    }
